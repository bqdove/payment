...
class CreatePayOrdersTable extends Migration
{
    ...
    public function up()
    {
        $this->schema->create('pay_orders', function (Blueprint $table) {
            ...
            $table->float('total_amount', 8,2)->comment('支付总额');//单位为人民币元
            $table->string('payment')->comment('付款方式');//wechat,alipay,unionpay
            $table->string('trade_no')->comment('各大支付平台提供的订单号');
            $table->string('pay_way')->comment('支付渠道');//微信扫码支付，支付宝网页支付等等
            $table->json('options')->comment('各大平台差异化数据');
//            $table->date('pay_time')->comment('支付时间');
            $table->timestamps();
        });
    }
